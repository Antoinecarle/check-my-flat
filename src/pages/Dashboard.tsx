...
const StatCard = ({ label, value, icon: Icon, accentColor }: StatCardProps) => (
  <div className="bg-white border border-slate-200 p-6 flex flex-col gap-4 relative overflow-hidden group">
    <div className="flex justify-between items-start">
      <div className={`p-2 border border-slate-100 ${accentColor}`}>
        <Icon size={20} className="stroke-[1.5px]" />
      </div>
      <span className="text-3xl font-light tracking-tight text-slate-900">{value}</span>
    </div>
    <div>
      <p className="text-[11px] uppercase tracking-wider font-semibold text-slate-500">{label}</p>
    </div>
    <div className="absolute bottom-0 left-0 w-full h-[1px] bg-slate-100 group-hover:bg-blue-600 transition-colors duration-500" />
  </div>
);
...
export default function Dashboard() {
  ...
  const activeBatiments = useMemo(() => batiments.filter(b => !b.archived).length, [batiments]);
  const activeLots = useMemo(() => lots.filter(l => !l.archived).length, [lots]);
  const edlThisMonth = useMemo(() => edls.filter(e => {
    if (!e.date_realisation) return false;
    const d = parseISO(e.date_realisation);
  ...
  const upcomingMissions = useMemo(() => {
    const future = missions
      .filter(m => {
        ...
        return m.statut === 'planifiee' && (isAfter(d, now) || format(d, 'yyyy-MM-dd') === format(now, 'yyyy-MM-dd'));
      })
      .sort((a, b) => a.date_planifiee.localeCompare(b.date_planifiee));

    if (role === 'technicien') {
      return future.filter(m => m.techniciens.includes(currentUserId));
    }
    return future;
  }, [missions, role, currentUserId]);

  const unassignedMissions = useMemo(() =>
    missions.filter(m => m.techniciens.length === 0 && m.statut === 'planifiee'),
  [missions]);
  ...
  const weekMissions = useMemo(() => {
    ...
    return missions.filter(m => {
      const d = parseISO(m.date_planifiee);
      return m.statut === 'planifiee' && isAfter(d, now) && isBefore(d, endOfWeek);
    });
  }, [missions]);

  const handleAssign = (missionId: string, techId: string) => {
    updateMission(missionId, { techniciens: [techId] });
  };
  ...
  return (
    <div>
      ...
      {/* Stats */}
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-px bg-slate-200 border border-slate-200 mb-12">
        <StatCard label="Batiments actifs" value={activeBatiments} icon={Building2} accentColor="text-blue-600" />
        <StatCard label="Lots geres" value={activeLots} icon={DoorOpen} accentColor="text-emerald-600" />
        <StatCard
          label={role === 'technicien' ? 'Mes missions a venir' : 'Missions cette semaine'}
          value={role === 'technicien' ? upcomingMissions.length : weekMissions.length}
          icon={CalendarCheck}
          accentColor="text-amber-600"
        />
        <StatCard label="EDL ce mois" value={edlThisMonth} icon={ClipboardList} accentColor="text-violet-600" />
      </section>

      {/* Content */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        <div className="lg:col-span-8 space-y-10">
          {/* Missions a attribuer — admin only */}
          ...
          <section>
            ...
            <div className="bg-white border border-slate-200 divide-y divide-slate-100">
              {upcomingMissions.length > 0 ? upcomingMissions.slice(0, 5).map(m => {
                ...
                return (
                  <Link key={m.id} to="/missions" className="group flex flex-col md:flex-row md:items-center justify-between p-4 hover:bg-slate-50 transition-all duration-200">
                    <div className="flex items-start gap-4 flex-1">
                      ...
                      <div className="flex flex-col">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] px-2 py-0.5 border rounded-full font-medium bg-blue-50 text-blue-600 border-blue-100">Planifiee</span>
                        </div>
